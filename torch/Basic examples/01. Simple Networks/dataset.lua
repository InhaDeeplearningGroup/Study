...
function mnist.loadDataset(fileName, maxLoad)
  ...
  if maxLoad and maxLoad > 0 and maxLoad < nExample then
    nExample = maxLoad
    print('[INFO] mnist dataset loading only ' .. nExample .. ' examples')
  end
  
  data = data[{ {1,nExample}, {}, {}, {} }]
  labels = labels[{ {1,nExample} }]
  print('[INFO] finish loading MNIST dataset')
  ...
  function dataset:normalize(mean, std)
    ...
    local std = std or data:view(data:size(1), -1):std(1, true)
  ...
  function dataset:normalizeGlobal(mean, std)
  ...
  local label_v = torch.zeros(10)
  
  setmetatable(dataset, {__index = function(self, index)
        local input = self.data[index]
        local class = self.labels[index]
        local label = label_v:zero()
        label[class] = 1
