...
class Env(tk.Tk):
    ...
    def _build_canvas(self):
        ...
        # 캔버스에 이미지 추가
        x, y = UNIT/2, UNIT/2
        self.rectangle = canvas.create_image(x, y, image=self.shapes[0], tags='my_rectangle')
    ...
    def load_images(self):
        ...
        rhombus = PhotoImage( # add shape
            Image.open("../img/rhombus.png").resize((30, 30)))

        return rectangle, triangle, circle, rhombus
    ...
    def check_if_reward(self, state):
        ...
        check_list['if_bonus'] = False
        rewards = 0

        for reward in self.rewards:
            if reward['state'] == state:
                if self.bonus and reward['reward'] == 10:
                    pass
                else:
    ...
    def step(self, action):
        ...
        s_ = self.get_state()
        if self.bonus:
            self.canvas.delete('my_bonus') # remove gui image 'bonus'
